...
TUTORIAL_SCREENSHOTS={
 ...
 "cpu-temperature-buzzer":["cpu-temperature-buzzer"],
...
def generic_body(t,locale="en"):
    ...
    details={
      ...
      "logics.overview":f'''<h2>How a workflow runs</h2><p>Data connectors carry current values. White Flow connectors trigger actions. Groups define independently controllable automations with Play, Pause, and Stop. Save and compile to embed the supported graph; the device web interface recreates it and overlays live values.</p><h2>Save changes made on the device</h2><ol><li>Edit the graph and resolve every red validation marker.</li><li>Select <strong>Save &amp; apply</strong> above the canvas. The success message must say that the graph will be restored after reboot.</li><li>Reload the page to confirm the same nodes and wires appear.</li><li>Reboot the device and confirm them again before relying on the automation.</li></ol><p>The current full 4 MiB USB layout reserves a 64 KiB internal LittleFS partition for editable Logics and also has a compact, chunked NVS fallback. The <em>Flash headroom</em> value describes unused bytes in the application slot; it is not the capacity available for graph files. An application-only OTA cannot replace the partition table, so a device using the older coredump layout needs one full USB installation before the internal filesystem becomes available.</p><h2>Recovery after a bad graph</h2><p>After a panic or watchdog restart during startup, saved Logics are quarantined and the compiled graph loads stopped. After the storage-safety migration, an existing saved graph is also restored stopped once. Review the graph, then deliberately press Play; ELMA asks for confirmation before retrying a quarantined automation.</p><h2>Contextual Help</h2><p>On Windows, right-click a workspace tab, Logics block, or automation group and choose Help / Documentation. In Android and the device web interface, right-click with a mouse or long-press the same item on a touch screen. ELMA-IoT opens the exact article for the active feature in the selected documentation language.</p><h2>Selecting and editing groups</h2><p>Click a group rectangle to highlight that group. Ctrl-click selects more than one group on Windows. Ungroup removes only the selected group frames and keeps their nodes and wires. Delete removes each selected group together with all nodes and wires inside it.</p><h2>Finding configured hardware</h2><p>Expand Active peripherals to find the capabilities generated from the current configuration. A configured Buzzer provides Peripheral reference, Play, and Stop. If it is missing after updating an older project, save the Audio Out selection again and compile the firmware so the device receives the refreshed peripheral contract.</p><h2>Learn in order</h2><ol><li><a href="{href(locale,'logics.thinking')}">Translate a real requirement into Flow and data</a>.</li><li><a href="{href(locale,'logics.connectors')}">Learn connector colors and compatibility</a>.</li><li><a href="{href(locale,'logics.state-timing')}">Use edges, state, and timing</a>.</li><li><a href="{href(locale,'building-devices')}">Combine several peripherals</a>.</li></ol>''',
      ...
      "flash.usb":"<h2>Safe sequence</h2><p>Select the correct serial device, write and verify all required regions, then allow the application to restart and acknowledge configuration. ELMA first tries the normal DTR/RTS reset sequence and, on supported native-USB chips, USB reset. The BOOT/RESET guide appears only after automatic entry fails. A verified image without configuration acknowledgement should be reconnected and checked rather than assumed complete.</p><h2>Procedure</h2><ol><li>Use a data-capable USB cable and select the correct port.</li><li>Compile for the exact board and chip.</li><li>Start flashing and let ELMA try automatic download-mode entry.</li><li>Only when prompted, hold BOOT, tap RESET, release BOOT, and retry without disconnecting the cable.</li><li>Keep the cable attached through MD5 verification, restart, and configuration acknowledgement.</li></ol><h2>Partition upgrade and saved settings</h2><p>A full USB installation writes the bootloader, partition table, OTA selection, and application. Current 4 MiB builds reserve the last 64 KiB as internal LittleFS storage for editable Logics. NVS is preserved unless the user explicitly selects erase.</p><h2>Common mistakes</h2><p>Charge-only cables, wrong ports, another program holding serial, unstable hubs, missing drivers, and incorrect boot mode are common causes. A successful write does not confirm that an incompatible pin configuration will boot safely.</p>",
      ...
      "troubleshooting.usb":'''<h2>Symptoms and causes</h2><p>No port usually means cable, permission, driver, connector, or hardware trouble. Failure to enter bootloader points to BOOT/RESET timing or a busy port. Write or verification failures often indicate power, signal integrity, flash mode, or wrong target.</p><h2>Diagnostic steps</h2><ol><li>Try a known data cable and direct USB port.</li><li>Close Serial Monitor and other serial programs.</li><li>Reconnect, rescan, select the port by its description, and start flashing normally.</li><li>Allow the normal and native-USB automatic reset attempts to finish.</li><li>Only if the app displays the board guide, hold BOOT, tap RESET, release BOOT, and retry.</li><li>Keep power stable through verification and reboot.</li></ol><h2>Recovering a boot loop</h2><p>If firmware crashes before automatic synchronization, manual ROM download mode may be unavoidable once. Back up NVS before writing when saved configuration matters, do not select full erase, and flash the current bootloader, partition table, OTA reset data, and application. The startup guard then keeps suspect saved Logics stopped so networking and OTA remain reachable.</p><h2>After a successful write</h2><p>If configuration is not acknowledged, reconnect and apply configuration instead of reflashing repeatedly. Read Serial Monitor for reset loops, pin conflicts, or power faults.</p>''',
